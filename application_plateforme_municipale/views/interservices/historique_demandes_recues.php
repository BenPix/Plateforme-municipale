<section class="mbr-section form3 cid-r1fWFz2mbZ" id="form3-c">
	...
	* {
	...
	<div style="display:none" id="options_de_filtre" class="container container_of_form">
	  <?php echo form_open('historique/demandes_recues'); ?>
	  <div class="row">
	    <div class="col-25">
	    ...
	    <div class="col-75">
	    	...
	    	$dossier_depart = array(
				'name' => 'dossier_depart',
				'value' => $this->session->flashdata('filtre_demandes_recues')['dossier_depart'],
				'placeholder' => 'ex. 159'
			);
	  ...
	  <div class="row">
	    ...
	    <div class="col-75">
	      <?php
	    	$dossier_fin = array(
				'name' => 'dossier_fin',
				'value' => $this->session->flashdata('filtre_demandes_recues')['dossier_fin'],
				'placeholder' => 'ex. 248'
			);
	  ...
	  <div class="row">
	    ...
	    <div class="col-75">
	      ...
	      $date_depart = array(
				'name' => 'date_depart',
				'value' => $this->session->flashdata('filtre_demandes_recues')['date_depart'],
				'type' => 'date'
			);
	  ...
	  <div class="row">
	    ...
	    <div class="col-75">
	      ...
	      $date_fin = array(
				'name' => 'date_fin',
				'value' => $this->session->flashdata('filtre_demandes_recues')['date_fin'],
				'type' => 'date'
			);
	  ...
	  <div class="row">
	    ...
	    <div class="col-75">
	      <?php echo form_dropdown('expediteur', html_escape($pole_attache), $this->session->flashdata('filtre_demandes_recues')['expediteur']); ?>
	    </div>
	  </div>
	  <div class="row">
	    <div class="col-25">
	      <label>Service sollicité :</label>
	    </div>
	    <div class="col-75">
	      <?php echo form_dropdown('sous_pole', html_escape($sous_pole), $this->session->flashdata('filtre_demandes_recues')['sous_pole']); ?>
	    </div>
	  </div>
	  <div class="row">
	    <div class="col-25">
	    ...
	    <div class="col-75" id="checkboxes">
	      ...
	      $checkbox1 = array(
	            'name'          => 'statut[]',
	            'value'         => 'en attente',
	            'checked'       => (NULL !== $this->session->flashdata('demandes_recues_en_attente')) ? $this->session->flashdata('demandes_recues_en_attente') : TRUE,
	        );
	        $checkbox2 = array(
	            'name'          => 'statut[]',
	            'value'         => 'en cours',
	            'checked'       => (NULL !== $this->session->flashdata('demandes_recues_en_cours')) ? $this->session->flashdata('demandes_recues_en_cours') : TRUE,
	...
	<div>
		...
		<!-- tableau des demandes reçues -->

		<?php if ( ! empty($tab_reception_avec_couleur) ) { ?>
			<p style="font-size: 20px;text-align: center;">
				<u>DEMANDES A VOTRE ÉGARD</u>
			</p>
			<?php
			echo $pagination.'<br>';

			$this->table->set_heading('N° Dossier', 'Horodateur', 'Direction attachée', 'Demande', 'Délai', 'Degré d\'urgence', 'Statut');

			$tmpl = array(
				'table_open' => '<table border="solid"; cellpadding="4" cellspacing="0" class="tab" id="tab_envoi">',
				'thead_open' => '<thead style="background-color: #DCDCDC; border: solid black 2px;">'
			);

			$this->table->set_template($tmpl);

			foreach ($tab_reception_avec_couleur->result() as $row) {
				// background de couleur selon la légende (affectation à un sous-pole)
				...
				// enrobage des demandes en rapport avec une notification
			...
			if ($tab_reception_avec_couleur->num_rows() > 0) {
